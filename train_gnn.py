...
def train(loader):
    ...
    with tqdm(loader, unit="batch") as tepoch:
        for data_ in tepoch:
            ...
            a = to_device(data_.x, device)
            b = to_device(data_.edge_index, device)
            c = to_device(data_.batch, device)
            d = to_device(data_.y.to(torch.float32), device) 
...
if __name__=='__main__':
    ...
    parser.add_argument('--kmer', type=int, default=3,
            help='select kmer on which model has to be trained (default: 3)')
    ...
    POSITIVE = DATAPATH+'processed_out/'+str(kmer)+'mer/positive/'
    NEGATIVE = DATAPATH+'processed_out/'+str(kmer)+'mer/negative/'
    ...
    for epoch in range(1, len(os.listdir(NEGATIVE))):
        ...
        if test_loss < best_loss:
            ...
            torch.save(model.state_dict(),
                       DATAPATH+'checkpoints/'+str(kmer)+'mer_'+TF+'_'+model_type+'_'+str(channels)+'_'+str(best_loss)+'.pt')
        ...
        # Save dict after every epoch
        np.save(DATAPATH+'results/'+str(kmer)+'mer/'+TF+'_'+model_type+'_'+str(channels)+'.npy', results)   
